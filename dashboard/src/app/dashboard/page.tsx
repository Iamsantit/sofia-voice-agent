...
export default async function DashboardPage() {
  ...
  const totalConv = stats.temperatura.hot + stats.temperatura.warm + stats.temperatura.cold;
  const hotPct = totalConv ? Math.round((stats.temperatura.hot / totalConv) * 100) : 0;

  return (
    <Shell>
      {/* Industry-aware welcome */}
      <IndustryWelcome />

      {/* Hero header with gradient + live info */}
      ...
      {/* KPI Hero cards — con gradientes y animated numbers */}
      ...
      {/* Second row: métricas + temperatura */}
  ...
  return (
    ...
    >
      ...
      <CardContent className="pt-6 relative">
        ...
        >
          <AnimatedNumber value={typeof value === "number" ? value : 0} fallback={String(value)} />
        </p>
      </CardContent>
    </Card>
  );
...
function StatCard({
  ...
  return (
    <Card className="border-white/[0.08] bg-gradient-to-br from-white/[0.03] to-transparent relative overflow-hidden">
      <div className={`absolute -top-8 -right-8 h-24 w-24 rounded-full ${glows[glow]} blur-2xl`} />
...
function StatusDot({ status }: { status: string }) {
  ...
  return (
    <div className={`h-2 w-2 rounded-full ${colors[status] || "bg-neutral-500"} shadow-[0_0_8px_currentColor]`} />
  );
}
